...
### Changed
- Each channel now carries its own logo: Claude Code and Codex show the tool's own icon instead of the Claude and OpenAI marks, in the provider menu, on the composer button, on the "generated by" tile of a quiz page and in the install dialog. claude.ai and chatgpt.com keep their brand logo, since a site is the brand.
