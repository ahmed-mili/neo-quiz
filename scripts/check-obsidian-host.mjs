...
await withSrcModule("apps/obsidian/host.ts", async ({ createObsidianHost }) => {
	...
	const attendu = {
		...
		/* PLUS de `pdf` : membre optionnel du contrat (`HostPdf`) que le greffon
		   ...
		   PLUS de `modals`/`net`/`process` non plus, retirés du même coup et
		   couverts par leur ABSENCE plus bas, pas par une liste de méthodes. */
	};
	const manquantes = [];
...
await withSrcModule("apps/obsidian/host.ts", async ({ createObsidianHost }) => {
	const r = makeReporter("Hôte Obsidian — icônes, et l'absence de modales/réseau/process");
	...
	/* `modals`, `net` et `process` sont partis à la tâche 2 du chantier
	   « greffon lecteur » avec leur seul consommateur, le tableau de bord —
	   ce sont désormais des membres OPTIONNELS du contrat (`src/host/types.ts`)
	   que CET hôte ne fournit PAS. Un `host.modals` qui reviendrait un jour
	   (« pour aller vite ») romprait `requireHost("modals")` en silence côté
	   code partagé : sans erreur nommée, juste un `undefined` qui plante
	   ailleurs, plus tard, sans dire pourquoi. */
	r.check("le greffon lecteur ne fournit pas de modales",
		"modals" in host && host.modals !== undefined, false);
	r.check("le greffon lecteur ne fournit pas de réseau",
...
await withSrcModule("apps/obsidian/host.ts", async ({ createObsidianHost }) => {
	...
	/* ── externe : plus de consommateur sous Obsidian ──

	   `HostFs.externe` reste un membre OBLIGATOIRE du contrat (l'application
	   le fournit encore, pour le même sélecteur « @ » que l'IA garde) ; le
	   greffon lecteur, lui, n'a plus le sélecteur qui s'en servait
	   (`src/dashboard/file-sources.ts`, `prompt-paths.ts`, retirés à la
	   tâche 1). L'implémentation ici n'est donc plus qu'un STUB qui ne touche
	   plus `fs`/Node : « rien à montrer », jamais un vrai dossier. */
	r.check("externe.list ne montre plus rien (plus d'appelant sous Obsidian)",
		await host.fs.externe.list(tmpdir()), []);
	r.check("externe.stat ne connaît plus rien", await host.fs.externe.stat(tmpdir()), null);
	let rejetLecture = false;
	try { await host.fs.externe.read(tmpdir()); } catch (e) { rejetLecture = true; }
	r.check("externe.read rejette (pas de disque sur cet hôte)", rejetLecture, true);
	let rejetLectureBinaire = false;
	try { await host.fs.externe.readBinary(tmpdir()); } catch (e) { rejetLectureBinaire = true; }
	r.check("externe.readBinary rejette (pas de disque sur cet hôte)", rejetLectureBinaire, true);
