...
def replace_once_or_done(path: str, old: str, new: str, done_marker: str) -> None:
    p = ROOT / path
    s = p.read_text(encoding="utf-8")
    if done_marker in s:
        return
    if s.count(old) != 1:
        raise SystemExit(f"{path}: motif unique introuvable pour {done_marker!r}")
    p.write_text(s.replace(old, new, 1), encoding="utf-8")


def remove_regex_once(path: str, pattern: str, old_marker: str) -> None:
    p = ROOT / path
    s = p.read_text(encoding="utf-8")
    ...
    if n != 1:
        raise SystemExit(f"{path}: bloc à supprimer introuvable pour {old_marker!r}")
    p.write_text(out, encoding="utf-8")


def regex_once_or_done(path: str, pattern: str, new: str, done_marker: str) -> None:
    p = ROOT / path
    s = p.read_text(encoding="utf-8")
    if done_marker in s:
        return
    ...
    if n != 1:
        raise SystemExit(f"{path}: motif regex unique introuvable pour {done_marker!r}")
    p.write_text(out, encoding="utf-8")


# ── Bootstrapper : pas de deuxième fenêtre, l'installeur reste visible. ──
...
# La suppression ci-dessus consomme l'ouverture du commentaire suivant : on la
# remet exactement une fois si nécessaire.
p = ROOT / "apps/windows/installer/main.ts"
s = p.read_text(encoding="utf-8")
needle = "\n * explicite `fenetre.prete()` envoyé après l'initialisation du rendu."
if needle in s and "/** L'application garde maintenant sa vraie fenêtre CACHÉE" not in s:
    s = s.replace(needle, "\n/** L'application garde maintenant sa vraie fenêtre CACHÉE" + needle, 1)
...
# ── Contrôle permanent : spinner dans l'installeur, aucune seconde fenêtre. ──
